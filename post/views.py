...
class UpdatePostView(LoginRequiredMixin, DataMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        formset = PostChannelFormSet(
            queryset=post_channels,
            prefix="postchannel_set",
            form_kwargs={
        ...
        sales_data = PostChannel.objects.filter(post__username=post.username).aggregate(
            first_purchase=Min("date_post"), last_purchase=Max("date_post")
        )
        post_channels = [
            v
            for v in post_channels
            if isinstance(v.tarif, (int, float))
            or (isinstance(v.tarif, str) and v.tarif.strip().replace(".", "", 1).isdigit())
        ]
    ...
    def post(self, request, *args, **kwargs):
        ...
        formset = PostChannelFormSet(
            request.POST,
            queryset=PostChannel.objects.filter(post=post),
            prefix="postchannel_set",
        ...
        errors_string = "\n".join(f"{field}: {', '.join(errors)}" for field, errors in post_form.errors.items())
...
class DetailPost(LoginRequiredMixin, DataMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        channel_post = PostChannel.objects.get(id=channel_post)
        formatted_time = channel_post.time_post.strftime("%H:%M")
        # Додаємо дані в контекст
        context["post"] = post
        context["title"] = (
            f"{self.title_page} для каналу {channel_post.channel.name} {channel_post.date_post} о {formatted_time}"
            if channel_post
            else self.title_page
        )
...
def check_time(request):
    ...
    if channel_id and time_post:
        ...
        if exists:
            channel_name = PostChannel.objects.get(
                channel_id=channel_id, time_post=time_post, date_post=date_post
            ).channel.name
...
def save_to_avtoposting(request, post_chanel_id):
    if request.method == "POST":
        ...
        post_channel.save()
        # HERE
        print(post_channel.channel)  # object channel - pk to model post.Channels
