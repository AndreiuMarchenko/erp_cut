...
class AddPostForm(forms.ModelForm):
    ...
    scrin = forms.ImageField(
        label="Скріншот підтвердження",
        required=False,
...
def PostChannelFormSet(*args, **kwargs):
    ...
    formset = inlineformset_factory(Post, PostChannel, form=AddPostToChannelsForm, can_delete=True, extra=1)(
        *args, **kwargs
    )
...
class ChannelForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["category"].choices = [("", "--- Оберіть категорію ---")] + [
            (id, name) for id, name in category_choices
        ]
